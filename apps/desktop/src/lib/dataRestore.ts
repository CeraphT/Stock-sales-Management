...
/** The rows in the file for a table, defensively filtered to the current company
 * on company-scoped tables (so a tampered file can never inject another
 * business's rows). */
...
export interface TableRecap {
  name: string;
  fileRows: number;
  /** How many of the file's rows already exist locally (matched by primary key). */
  existing: number;
  newRows: number;
}
...
): Promise<{ companyMismatch: boolean; recap: TableRecap[] }> {
 ...
 * - `mode: "replace"` — upsert: existing matches are overwritten with the
 *   backup's version, new rows inserted.
 * Statements run individually (no wrapping transaction) — matches how the sync
 * mirror writes on this driver.
 */
