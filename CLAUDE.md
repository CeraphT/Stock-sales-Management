...
- **PostgreSQL 17** running as a Windows service, db `pharmastock`,
  `postgres`/`postgres` dev credentials (see `appsettings.json`)
- **.NET SDK 10.0.103** installed (only SDK present) — Domain/Infrastructure/Api
  target `net8.0` (builds fine cross-targeted); Desktop/Mobile targets
  `net10.0-android` + `net10.0-windows10.0.19041.0`
...
- **Android SDK**: `C:\android-sdk` (set `$env:ANDROID_HOME`)
- **JDK**: `C:\microsoft-jdk` (set `$env:JAVA_HOME`)
- Both env vars must be set per-session (not persisted globally yet) before
  any `dotnet build -f net10.0-android` or `adb`-adjacent command
- Physical Android tablet connects via USB; after each reconnect, redo:
  `& "C:\android-sdk\platform-tools\adb.exe" reverse tcp:5080 tcp:5080`
- To run the API: `cd src/PharmaStock.Api && dotnet run --urls http://localhost:5080`
- To deploy to the tablet: `dotnet build src/PharmaStock.Desktop/PharmaStock.Desktop.csproj -f net10.0-android -t:Run`
