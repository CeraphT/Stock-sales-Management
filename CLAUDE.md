...
- **Client — Desktop**: **`apps/desktop/` — Tauri v2 + React + TS + Tailwind, BUILT (2026-08-03).** Consumes `@stockflow/core`; dual-driver local SQLite (tauri-plugin-sql natively, sql.js in a plain browser for drivable dev/testing) both via drizzle `sqlite-proxy`; full bidirectional sync. Every screen is real: auth, Dashboard, Products, POS/checkout (FEFO + offline push), Sales History + Sale Detail (+refund), Categories, Product/Stock CRUD, Suppliers, Customers (+loyalty), Gift Cards, Purchase Orders, Reports, Services, Staff, Company Settings, and HTML/PDF receipt printing. Only the raw **ESC/POS thermal transport** (a native Rust serial/USB/network command over `@stockflow/core/printer/escpos`) is unbuilt — hardware-dependent. Rebuild guide (still accurate for architecture): **[`docs/desktop-rebuild.md`](docs/desktop-rebuild.md)**. Dev: API needs the CORS policy (added in `Program.cs`); `cd apps/desktop && npm run tauri dev` (native) or `npm run dev` (browser at :5173).
